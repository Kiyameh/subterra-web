...
/**
 * @schema de Zod para validar formulario de creación de cueva
 */
...
export const CaveFormSchema = z.object({
  ...
  catalog: z
    .string()
    .max(caveMaxCharacters.catalog, {message: 'Máximo 10 caracteres'})
    .optional(),
  initials: z
    .array(
      z
        .string()
        .max(caveMaxCharacters.initials, {message: 'Máximo 10 caracteres'})
    )
    .optional(),
  name: z
    .string()
    .min(3, {message: 'Nombre requerido'})
    .max(caveMaxCharacters.name, {message: 'Máximo 40 caracteres'}),
  alt_names: z
    .array(
      z
        .string()
        .max(caveMaxCharacters.alt_names, {message: 'Máximo 40 caracteres'})
    )
    .optional(),
  //* Descripciones:
